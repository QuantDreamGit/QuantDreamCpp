...
RT winsorized_mean(Eigen::MatrixBase<Derived>& data, double const& trim_fraction) {
  ...
  data.head(k).setConstant(data(k));
  data.tail(k).setConstant(data(data_size - k - 1));

  // Compute the mean of the winsorized data
  RT winsorized_mean = data.mean();
