...
namespace qd::statistics::robust::center {
  ...
  RT trimmed_mean(Eigen::MatrixBase<Derived> const& data, double const& trim_fraction) {
    ...
    RT trimmed_mean = data_copy.segment(k, data_size - 2 * k).mean();
  ...
  RT trimmed_mean(std::vector<T> const& data, double const& trim_fraction) {
